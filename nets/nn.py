...
class CrossEntropyLoss2(torch.nn.Module):
    """
    [https://arxiv.org/pdf/1512.00567.pdf]
    """

    def __init__(self, epsilon=0.01,weight=None):
        super().__init__()
        self.epsilon = epsilon
        self.softmax = torch.nn.LogSoftmax(dim=1)
        self.crossentloss = torch.nn.CrossEntropyLoss(weight=weight,ignore_index =-1)

    def forward(self, x, target):
        prob = self.softmax(x)
        loss = self.crossentloss(x,target)
        return ((1. - self.epsilon) * loss + self.epsilon * (-prob.mean(dim=1))).mean()
